...
#In this version we will de-bias both the estimate and the bootstrap estimates


#' Simulate expected coverage of several alternative intervals given a set of parameter values.
#' @description This function can be used to generate the example shown in figure 1.
#' Simulated parameter estimates are ranked w.r.t absolute value however, bootstrap
#' confidence intervals can be calculated ranked either by absolute value or not.
...
example_sim <- function(theta, n=200, use.abs=TRUE){
  ...
  Z <- matrix(nrow=p, ncol=n)
	simnames <- c("par", "wfb", "wfb2", "oracle", "naive")
  ...
  cat("\n")
  return(list("Z"=Z, "COVERAGE"=COVERAGE, "WIDTH"=WIDTH, "simnames"=simnames))
}
