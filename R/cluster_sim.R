...
cluster_sim <- function(beta, Sigma, err.sd=1, n.samp=100, n.rep=100, seed=NULL, parallel=FALSE){
  ...
  while(i <= n.rep){
    ...
    #Ranking functions
    rk.basic <- basic_rank(res.orig$statistic, use.abs=TRUE)
    ...
    for(jj in 1:2){
      nm <- paste0("wfb2_", nms[jj])
      COVERAGE[which(simnames==nm), ixs[[jj]] ,i] <- (ci.wfb.2[,1] < effects & effects < ci.wfb.2[,2])[orders[[jj]]]
      WIDTH[which(simnames==nm), ixs[[jj]], i] <- (ci.wfb.2[,2] -ci.wfb.2[,1])[orders[[jj]]]
    }
