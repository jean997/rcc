...
#' }
#'@export
example_sim <- function(theta, n=200, use.abs=TRUE, bs.rep=20){
  p <- length(theta)
  Z <- matrix(nrow=p, ncol=n)
	simnames <- c("par", "par_db", "wfb", "wfb2", "oracle", "naive", "selInf1")
  ...
  cat("\n")
  return(list("Z"=Z, "COVERAGE"=COVERAGE, "WIDTH"=WIDTH, "simnames"=simnames, "INTERVALS"=INTERVALS, "THETA"=THETA))
}
