...
In this document we will compare several different approaches to building confidence intervals in high dimensions using the example in Section 1.5 of "Rank conditional coverage and confidence intervals in high dimensional problems." At the end of this document you can find the code that generates the results shown in the paper exactly. We will make use of the `rcc` package which accompanies the paper and can be found at [github.com/jean997/rcc](https://github.com/jean997/rcc).

## Generate data
For thisr exploration we will use setting 2 from section 1.5. We have 1000 parameters generated from a $N(0, 1)$ distribution. Each of our observed statistics $Z_i$ is generated as $$Z_i \sim N(\theta_i, 1).$$
...
library(ggplot2)
library(rcc)
plot_cis <- function(rank, ci, truth, prop=1){
...
plot_cis(rank, ci.naive, theta) + ggtitle("Standard Marginal Intervals")
```
...
plot_cis(rank[rank <= 100], ci.by, theta.sel) + ggtitle("Benjamini and Yukatieli Intervals")
```
...
For the  @Weinstein2013 intervals, we use code that accompanied that paper. This code is included in the `rcc` package for convenience. These intervals are asymetric and narrower than the @Benjamini2005 intervals but still control the average coverage in the selected set. 
...
```
```{r, show=FALSE, hold=TRUE, echo=FALSE, out.width="0.5\\textwidth"}
plot_cis(rank[rank <= 100], ci.wfb, theta.sel) + ggtitle("Weinstein, Fithian, and Benjamini Intervals")
```

Unforntunately, most of the non-covering intervals are still for the parameters with the most significnt estimates. This pattern is clear when we look at the average coverage over 100 data sets:
```{r, cache=TRUE}
...
```{r, show=FALSE, hold=TRUE, echo=FALSE, out.width="0.5\\textwidth"}
plot_cis(rank[rank <= 100], ci.rtt, theta.sel) + ggtitle("Reid, Taylor, and Tibshirani Intervals")
```
...
#ci.boot <- rcc::par_bs_ci(z=Z)
sum(ci.boot[,1] <= theta & ci.boot[,2] >= theta)/1000
```


```{r, show=FALSE, hold=TRUE, echo=FALSE, out.width="0.5\\textwidth"}
plot_cis(rank, ci.boot, theta) + ggtitle("Parametric Bootstrap Intervals")
```
...
system.time(boot.coverage <- apply(zsim, MARGIN=2, FUN=function(zz){
  j <- order(abs(zz), decreasing=TRUE)
  B <- replicate(n = 500, expr = {
    w <- rnorm(n=1000, mean=zz, sd=1)
    k <- order(abs(w), decreasing = TRUE)
    sign(w[k])*(w[k]-zz[k])
  })
  qs <- apply(B, MARGIN=1, FUN=function(x){quantile(x, probs=c(0.05, 0.95))})
  ci <- cbind(zz[j]-qs[2,], zz[j]-qs[1,])
  which.neg <- which(zz[j] < 0)
  ci[ which.neg , ] <- cbind(zz[j][which.neg] + qs[1,which.neg], zz[j][which.neg]+qs[2,which.neg])
  #ci <- rcc::par_bs_ci(z=zz)
  covered <- (ci[,1] <= theta[j] & ci[,2] >= theta[j])
  return(covered)
}))
#The average coverage in each simulation is close to the nominal level
...
oracle.coverage <- apply(zsim, MARGIN=2, FUN=function(zz){
  j <- order(abs(zz), decreasing=TRUE)
  B <- replicate(n = 500, expr = {
    w <- rnorm(n=1000, mean=theta, sd=1)
    k <- order(abs(w), decreasing = TRUE)
...
titles <- c("All Zero", "All N(0, 1)", "100 Effects=3", "100 Effects N(0, 1)")
```
This matrix is also included as a builtin data set in the `rcc` package.
We generate simulation results using the `example_sim` function:
...
These results are also included as a built-in data set to the `rcc` package.
Now we can make plots using the `plot_coverage` and `plot_width` functions in the `rcc` package. 
...
for(i in 1:4){
  ...
  covplots[[i]] <- plot_coverage(sim.list[[i]], proportion=0.2,
                               ...
                               legend.names = c("Marginal", "Parametric Bootstrap", "Oracle", "ashr", "WFB", "RTT"), 
...
ggsave(h1, file="~/Dropbox/rcc-temp/for_jcgs/img/example_cov.pdf", heigh=4, width=16, units="in", dpi=300)
ggsave(h2, file="~/Dropbox/rcc-temp/for_jcgs/img/example_width.pdf", heigh=4, width=16, units="in", dpi=300)
ggsave(legend$plot, file="~/Dropbox/rcc-temp/for_jcgs/img/example_legend.pdf", height=1, width=8, units="in", dpi=300)
```
